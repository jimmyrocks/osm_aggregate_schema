  ...
  FROM (
    ...
    FROM (
      SELECT
        ...
        role
      FROM
        (SELECT unnest(geom) as geom, unnest(role) as role from o2p_aggregate_relation(3351325)) z
      WHERE
        role != 'inner' AND
        ST_IsClosed(geom)
      ...
      SELECT
        ...
        role
      FROM
        (SELECT unnest(geom) as geom, unnest(role) as role from o2p_aggregate_relation(3351325)) x
      WHERE
        role = 'inner' AND
        ST_IsClosed(geom)
